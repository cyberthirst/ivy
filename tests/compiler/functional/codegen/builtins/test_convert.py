# adapted from: https://github.com/vyperlang/vyper/blob/097aecfe29e6c218d165d223f0272265cf6f5403/tests/functional/builtins/codegen/test_convert.py#L34
...
from ivy.abi import abi_decode, abi_encode, EncodeError, DecodeError

BASE_TYPES = (
    set(IntegerT.all()) | set(BytesM_T.all()) | {DecimalT(), AddressT(), BoolT()}
)
...
def _from_bits(val_bits, o_typ):
    ...
    try:
        ret = abi_decode(o_typ, val_bits, ivy_compat=False)
...
@pytest.mark.fuzzing
def test_convert_passing(get_contract, i_typ, o_typ, val):
...
def test_flag_conversion(get_contract, val, typ):
...
def test_flag_conversion_2(get_contract, tx_failed, val, typ):
    contract = f"""
flag Status:
    STARTED
    PAUSED
...
@pytest.mark.parametrize(
    "cls1,cls2", itertools.product((StringT, BytesT), (StringT, BytesT))
)
...
@pytest.mark.fuzzing
def test_conversion_failures(get_contract, tx_failed, i_typ, o_typ, val):
    """
