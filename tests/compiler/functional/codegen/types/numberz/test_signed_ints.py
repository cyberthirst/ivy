...
def test_exponent_negative_power_runtime_check(get_contract, base):
    # #2985
    code = f"""
@external
...
@pytest.mark.parametrize("typ", types)
def test_exponent(get_contract, typ):
    typ = sorted(IntegerT.signeds())[0]
...
@pytest.mark.parametrize("typ", types)
def test_num_bound(get_contract, typ):
    lo, hi = typ.ast_bounds
    ...
    for x, y in itertools.product(xs, ys):
        ...
        if ok:
            ...
            with pytest.raises(ZeroDivisionError):
                c.foo(x, y)
            # NOTE: we don't care about compilation errors
...
@pytest.mark.parametrize("typ", types)
def test_negation(get_contract, typ):
    code = f"""
@external
def foo(a: {typ}) -> {typ}:
    return -a
