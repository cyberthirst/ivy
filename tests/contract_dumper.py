...
def dump_contract(compiler_data, ctor_args: str, calldata_hex: str):
    ...
    for rec in records:
        if rec.get('source') == source_code and rec.get('ctor_args', '') == ctor_args:
            ...
            if calldata_hex and calldata_hex not in rec.get('calldatas', []):
                print(f"Adding calldata for contract: {calldata_hex[:30]}...", file=sys.stderr)
    ...
    print(f"Adding new contract entry with calldata: {calldata_hex[:30]}...", file=sys.stderr)
