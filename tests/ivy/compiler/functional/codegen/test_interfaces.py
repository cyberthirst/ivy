...
def transfer(to: address, amount: uint256):
    ...
    input_bundle = make_input_bundle(
        {"token.vy": token_code, "itoken.vyi": token_interface}
    )
...
def test_external_call_to_interface_kwarg(
    get_contract, kwarg, typ, expected, make_input_bundle
):
    interface_code = f"""
@external
@view
...
def test_fail3() -> Bytes[3]:
    ...
    bad_json_interface = json.dumps(
        compile_code(external_contract, output_formats=["abi"])["abi"]
    )
