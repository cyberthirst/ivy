...
def test_decimal_override():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        assert (
            str(w[-1].message)
            == "Changing decimals precision could have unintended side effects!"
        )
...
def _num_mul(x: decimal, y: decimal) -> decimal:
    ...
    assert c._num_mul(
        decimal_to_int(x), decimal_to_int(1 - DECIMAL_EPSILON)
    ) == decimal_to_int(quantize(x * (1 - DECIMAL_EPSILON)))
