...
def _fail_contract(code, exceptions):
...
@given(
    start=_draw_1024,
    length=_draw_1024,
    length_bound=_draw_1024_1,
    bytesdata=_bytes_1024,
)
...
def do_splice() -> Bytes[{length_bound}]:
    ...
    if (
        ...
        _fail_contract(code, ArgumentException)
...
# TODO investigate why the tests are failing
@pytest.mark.skip
...
def test_slice_bytes32_calldata_extended(get_contract, code, result):
    ...
    assert (
        c.bar(
            3, "0x0001020304050607080910111213141516171819202122232425262728293031", 5
        ).hex()
        == result
    )
...
def test_slice_buffer_oob_reverts(bad_code, get_contract, tx_failed):
    ...
    assert "Slice out of bounds" in str(e.value)


# tests all 3 adhoc locations: `msg.data`, `self.code`, `<address>.code`
# TODO: enable code tests once implemented
# @pytest.mark.parametrize("adhoc_loc", ["msg.data", "self.code", "a.code"])
@pytest.mark.parametrize("adhoc_loc", ["msg.data"])
