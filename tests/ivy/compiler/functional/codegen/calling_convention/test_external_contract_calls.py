...
from vyper.semantics.types import AddressT, TupleT, DArrayT, StringT

from ivy.abi import abi_encode, DecodeError
...
def test(addr: address) -> (int128, address, Bytes[10]):
    ...
    assert c1.out_literals() == (
        1,
        "0x0000000000000000000000000000000000000123",
        b"random",
    )
    assert c2.test(c1.address) == (
        1,
        "0x0000000000000000000000000000000000000123",
        b"random",
    )
...
@pytest.mark.parametrize(
    "i,ln,s,", [(100, 6, "abcde"), (41, 40, "a" * 34), (57, 70, "z" * 68)]
)
...
@pytest.mark.skip(reason="selfdestruct not implemented")
...
def foo(a: {typ}):
    ...
    encoded = abi_encode((TupleT((AddressT(),))), (val,)).hex()
...
def foo(a: DynArray[{typ}, 3], b: String[5]):
    ...
    valid = data[:138]
    # TODO ivy uses the strict abi-decoder and thus this call fails
    # oth vyper uses non-strict decoder for calldata and thus there
    # this call passes
