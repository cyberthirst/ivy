...
BAD_CODE = [
    code if isinstance(code, tuple) else (code, StructureException) for code in BAD_CODE
]
for_code_regex = re.compile(r"for .+ in (.*):")
bad_code_names = [
    f"{i} {for_code_regex.search(code).group(1)}"
    for i, (code, _) in enumerate(BAD_CODE)
]
