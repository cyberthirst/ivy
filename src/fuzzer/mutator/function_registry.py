...
from typing import Optional, Dict, List, Set, Tuple, Type
...
class FunctionRegistry:
    ...
    def get_compatible_builtins(
        ...
        for name, b in self.builtins.items():
            ...
            if name == "convert":
                if self.convert_target_supported(return_type):
    ...
    @classmethod
    def convert_target_supported(cls, dst: VyperType) -> bool:
        return type(dst) in _CONVERT_RULES

    @classmethod
    def convert_source_kinds(cls, dst: VyperType) -> tuple[Type[VyperType], ...]:
        ...
        *,
        ...
        if same_type:
            if not allow_same_type:
                return False
            if not cls._convert_same_type_allowed(src, dst):
                return False
