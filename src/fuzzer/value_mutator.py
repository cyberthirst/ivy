...
class ValueMutator:
    ...
    def generate_value_for_type(self, vyper_type: VyperType) -> Any:
        ...
        if isinstance(vyper_type, TupleT):
            return tuple(self.generate_value_for_type(t) for t in vyper_type.member_types)
        
        if isinstance(vyper_type, StructT):
            return tuple(self.generate_value_for_type(t) for t in vyper_type.tuple_members())
        
        if isinstance(vyper_type, SArrayT):
            return [self.generate_value_for_type(vyper_type.value_type) for _ in range(vyper_type.length)]
        
        if isinstance(vyper_type, DArrayT):
            n = self.rng.randint(0, vyper_type.length)
            return [self.generate_value_for_type(vyper_type.value_type) for _ in range(n)]
    ...
    def get_boundary_values(self, vyper_type: VyperType) -> List[Any]:
        ...
        if isinstance(vyper_type, IntegerT):
            ...
            return ["", "a", "A" * min(100, vyper_type.length), "\x00", "test", "0x1234"]
    ...
    def _generate_string(self, max_length: int) -> str:
        ...
        boundary_values = [s for s in self.get_boundary_values(StringT(max_length)) 
    ...
    def _mutate_string(self, value: str, max_length: int) -> str:
        ...
        if mutation_type == "add_char" and len(value) < max_length:
            ...
            char = self.rng.choice('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
            ...
            pos = self.rng.randint(0, len(value) - 1)
            chars = list(value)
            chars[pos] = self.rng.choice('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
            return ''.join(chars)
    ...
    def _generate_bytes(self, max_length: int) -> bytes:
        ...
        boundary_values = [b for b in self.get_boundary_values(BytesT(max_length)) 
